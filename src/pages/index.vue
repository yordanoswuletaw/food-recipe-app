...
<template>
  ...
  >
    ...
    <h1 class="text-2xl font-semibold text-white text-opacity-95">
      Visite • Create • Save Recipes
    </h1>
  </div>

  <h1
    class="text-2xl w-full text-center my-1.5 font-bold text-gray-900 text-opacity-95"
  >
    <span class="text-orange-400">Categories </span>{{ currentCategory }}
  </h1>

  <div
    class="relative group overflow-hidden w-full py- flex items-center justify-center"
  >
    <button type="button">
      <IconBackArrow
    ...
    >
      ...
      >
        <h3
          :class="{
            active_category: category.name === currentCategory,
  ...
  <RecipeNotFound
    v-else-if="error || filterRecipesResult?.length <= 0"
