...
<template>
  ...
  >
    <img
      class="w-full h-[8.5rem] rounded-t-lg object-cover"
    ...
    <div class="px-3 py-2">
      <h5
        class="text-gray-900 w-full truncate mb-2 text-md font-bold tracking-tight break-all"
      >
        {{ recipe.title }}
      </h5>

      <p
        class="text-sm overflow-hidden mb-0.5 w-full h-[2.4rem] text-black break-all"
      >
        {{
    ...
    <div class="flex px-2 py-1 items-center justify-evenly flex-wrap">
      <div
        class="w-fit mr-0.5 h-fit flex items-center text-dark_red rounded-full p-1"
      >
        <IconFav class="w-4 h-4" :fill="recipe.is_liked" />
        <p class="pl-1 text-sm font-medium">{{ recipe.total_like }}</p>
      </div>
      <div
        class="w-fit mr-0.5 h-fit flex items-center text-orange-400 rounded-full p-1"
      >
        <IconStar class="w-4 h-4 mr-0.5" :fill="recipe.is_rated" />
        <p class="pl-0.5 text-sm font-medium">
          {{
            recipe.total_rate == null
              ? 0
              : Number.isInteger(recipe.total_rate)
              ? recipe.total_rate
              : recipe.total_rate.toFixed(1)
          }}
        </p>
      </div>
      <div
        class="w-fit mr-0.5 h-fit flex items-center text-gray-900 rounded-full p-1"
      >
    ...
    <div class="flex items-center justify-start min-w-full">
      ...
      >
        Details
        <svg
          class="ml-2 -mr-1 w-4 h-4 sm:block hidden"
          fill="currentColor"
          viewBox="0 0 20 20"
      ...
      <div
        class="text-gray-900 font-semibold text-sm px-0.5 h-fit flex items-center rounded-full justify-center"
      >
